...
load_classphp('./TimeTracking');

//-- Setup config
if (file_exists('TimeTracking/config/config.php')) {
    include './TimeTracking/config/config.php';
} else {
    include './TimeTracking/config/default.config.php';
}
print_r($config);
//-- Test to see if there is a json file
$jsonFileObject = new TimeTracking\Model\JsonFile($config);
print $jsonFileObject->getJsonFile();
